...
from services.rendering.redaction.redaction_analysis import collect_page_intrusive_display_text_rects
...
from services.rendering.redaction.redaction_analysis import rect_intersects_intrusive_display_text
...
def apply_standard_redaction(
    ...
    *,
    ...
    candidate_rects = [rect for rect, _item, _translated_text in valid_items]
    intrusive_display_rects = collect_page_intrusive_display_text_rects(page)
    for rect, item, _translated_text in valid_items:
        if fill_background is None:
            removable_rects = item_removable_text_rects(
                ...
                competing_rects=candidate_rects,
            )
            if removable_rects:
                if any(
                    rect_intersects_intrusive_display_text(removable_rect, intrusive_display_rects)
                    for removable_rect in removable_rects
                ):
                    cover_rects.extend(removable_rects)
                    continue
                redactions.extend((removable_rect, None) for removable_rect in removable_rects)
                continue
